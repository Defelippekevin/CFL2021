...
let arrPalabras: string[] = new Array(8); 

for (let i: number = 0; i < arrPalabras.length; i++) {

    arrPalabras[i] = ReadlineSync.question(" Ingrese una palabra ");
...
function ejecutarComando(comandoElegido:number):void{
 ...
 function actualizarPalabra(): void {
    ...
    let posicionAInsertar: number;
    let palabraAInsertar: string;
